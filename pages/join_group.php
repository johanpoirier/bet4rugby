...
<div class="maincontent">
    <div class="ppp">
        ...
        <form method="post" id="join_group_form"  name="join_group_form" action="/?op=join_group">
            <input type="hidden" name="code" id="code" <?php if($code) { echo 'value="'. $code . '"'; } ?>/>
            <br/>
            <div class="formfield"><b>Sélectionner le groupe que vous souhaitez rejoindre</b></div>
            <select name="group" id="group">
                <?php foreach ($userTeams as $userTeam) { ?>
                    <option name="<?php echo $userTeam['userTeamID']; ?>" value="<?php echo $userTeam['userTeamID']; ?>"><?php echo $userTeam['name']; ?> (<?php echo $userTeam['ownerName']; ?>)</option>
                <?php } ?>
            </select>
            <br /><br />
